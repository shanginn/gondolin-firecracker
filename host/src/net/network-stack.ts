...
export type NetworkStackOptions = {
  ...
  sshPorts?: number[];
  /** network tx buffer hard cap in `bytes` (includes the 4-byte length prefix) */
 ...
 * because the network TX queue hit its hard cap.
 */
...
export class NetworkStack extends EventEmitter {
  ...
  // Called when network writes data to the network interface
  ...
  writeFrameToNetwork(frame: Buffer) {
    this.receive(frame);
  }

  // Called when network wants to read data from the network interface
  ...
  readFrameFromNetwork(): Buffer | null {
    if (this.txQueueSize === 0) return null;
    ...
    const frameLen = packet.readUInt32BE(0);
    if (packet.length < 4 + frameLen) return null;
    return packet.subarray(4, 4 + frameLen);
  }
  ...
  /**
   * Enqueues a network-framed ethernet packet for host->guest delivery.
   *
   * Emits:
  ...
  private enqueueTx(packet: Buffer, priority: TxPriority): boolean {
    ...
    // Enforce a hard cap to avoid unbounded memory growth if network stops draining.

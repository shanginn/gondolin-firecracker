...
test("SandboxServer: stop->exit emits enriched inflight error (not generic)", async () => {
  ...
  controller.emit("log", "firecracker: badness\n", "stderr");
  ...
  assert.deepEqual(captured.json[0], {
    ...
    message: "sandbox exited (code=1) (firecracker: firecracker: badness)",
  });
});

test("SandboxServer: VMM hint does not include stdout (guest console) and strips ANSI", async () => {
  const server = new SandboxServer(makeResolvedOptions());
  (server as any).bridge.send = () => true;
  ...
  controller.emit("log", "firecracker: oops \u001b[31mred\u001b[0m\n", "stderr");
  ...
  assert.ok(msg.includes("(firecracker:"));
  assert.ok(!msg.includes("SECRET"));
  assert.ok(!msg.includes("\u001b"));
  assert.ok(msg.includes("firecracker: oops red"));
});

test("SandboxServer: startup diagnostic includes partial VMM stderr", () => {
  const server = new SandboxServer(makeResolvedOptions());
  const controller = (server as any).controller;

  controller.emit("log", "firecracker: partial failure", "stderr");

  assert.equal(
    (server as any).getStartupDiagnostic(),
    " (firecracker: firecracker: partial failure)",
  );
});
...
test("SandboxServer: VMM hint skips low-value Zig stack frames", async () => {
  const server = new SandboxServer(makeResolvedOptions());
  (server as any).bridge.send = () => true;
  ...
  controller.emit("log", "error: firecracker start failed: rc=-22\n", "stderr");
  controller.emit("log", "error: FirecrackerError\n", "stderr");
  controller.emit(
    "log",
    "/tmp/gondolin/host/firecracker/main.zig:158:23: 0x100393f7f in runVm\n",
    "stderr",
  );
  controller.emit(
    "log",
    "    if (start_rc < 0) return startError(start_rc);\n",
    "stderr",
  );
  controller.emit("log", "                       ^\n", "stderr");
  controller.emit(
  ...
  assert.equal(
    ...
    "sandbox exited (code=1) (firecracker: error: firecracker start failed: rc=-22)",
  );
});

test("SandboxServer: VMM hint tail is cleared on starting", async () => {
  const server = new SandboxServer(makeResolvedOptions());
  (server as any).bridge.send = () => true;

  const controller = (server as any).controller;

  controller.emit("log", "firecracker: old\n", "stderr");
  controller.emit("state", "starting");
  controller.emit("log", "firecracker: new\n", "stderr");
  ...
  assert.ok(message.includes("firecracker: firecracker: new"));
